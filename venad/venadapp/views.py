...
from sipbuild.generator.specification import License
from .models import *
...
def delete_row_route(request, id):
    try:
...
def employeemaster(request):
    emp = Employee.objects.all()  # Fetch all financial years
    context = {
        'employeemaster': emp
    }
    return render(request, 'employeemaster.html', context)


def employeemasteraddview(request):
    return render(request, "employeemasteradd.html")
...
def add_employee(request):
    ...
    try:
        ...
        govt_id = request.FILES['governmentID']
        fs1 = FileSystemStorage()
        file1 = fs1.save(govt_id.name, govt_id)
        url1 = fs1.url(file1)
        obj.govt_id = url1
        obj.save()
    except:
        obj.employee_id = request.POST.get("employeeID")
        obj.name = request.POST.get("name")
        obj.blood_group = request.POST.get("bloodGroup")
        obj.designation = request.POST.get("designation")
        obj.address = request.POST.get("address")
        obj.phone = request.POST.get("phone")
        obj.phone_2 = request.POST.get("phone2")
        obj.email = request.POST.get("email")
        obj.emp_type = request.POST.get("employeeType")
        obj.doj = request.POST.get("doj")
        obj.basic_salary = request.POST.get("basicSalary")
        obj.ac_number = request.POST.get("accountNumber")
        obj.save()
...
def employee_edit_page(request, id):
    obj = get_object_or_404(Employee, pk=id)
    context = {
        'emp': obj
    }
    return render(request, 'employeemasteredit.html', context)


def employee_edit_save(request):
    id = request.POST.get("id")
    obj = get_object_or_404(Employee, pk=id)
    try:
        photo = request.FILES['employeeImage']
    ...
    try:
        govt_id = request.FILES['governmentID']
        fs = FileSystemStorage()
        file = fs.save(govt_id.name, govt_id)
        url = fs.url(file)
        obj.govt_id = url
        obj.save()
    except:
...
def member_edit_page(request, id):
    ...
    return render(request, 'memberedit.html', {'mem': obj})
...
def Attendance_Registerview(request):
    obj = Employee.objects.all()
    return render(request, "Attendance_Register.html", {'att': obj})


def mark_attendance(request, id):
    d = Employee.objects.get(id=id)
    return render(request, "mark_attendance.html", {"d": d})


def save_attendance(request, id):
    obj = Attendance()
    obj.employee_id = id
...
def prodAdd(request):
    ...
    return render(request, "productadd.html",{"data":data,"data1":data1})
...
def advpayment_view(request):
    obj = Adv_payment.objects.all()
...
def advance_paymentadd(request):
    obj_emp = Employee.objects.all()
    context = {
        'emp': obj_emp
    }
    return render(request, "advance_payment_add.html", context)
...
def ajax_view(request):
    if request.method == 'POST':
        ...
        obj = get_object_or_404(Employee, pk=selected_option)
        basic_sal = obj.basic_salary
...
def save_advancepayment(request):
    ed = request.POST.get("ed")
    if (ed == "edit"):
        id = request.POST.get("id")
        obj = get_object_or_404(Adv_payment, pk=id)
        obj.date = request.POST.get("date")
        obj.amount = request.POST.get("amount")
        obj.remainingbalence = request.POST.get("remaining-balance")
        obj.save()
    else:
        obj = Adv_payment()
        selected_option = request.POST.get("employee-name")
        obj.employee = get_object_or_404(Employee, pk=selected_option)
        obj.date = request.POST.get("date")
        obj.amount = request.POST.get("amount")
        obj.remainingbalence = request.POST.get("remaining-balance")
        obj.save()
...
def advpayedit(request):
    id = request.POST.get("id")
    obj = get_object_or_404(Adv_payment, pk=id)
    context = {
        "adv": obj
    }
    return render(request, "advedit.html", context)
...
def overtime_details(request):
    overtimes = Overtime.objects.all()  # Fetch all overtime records
    context = {
        'overtimes': overtimes
    }
    return render(request, 'overtime_details.html', context)
...
def overtime_add(request):
    ...
    return render(request, 'overtimeadd.html',context)

def overtime_save(request):
    obj = Overtime()
    obj.employee_id = request.POST.get("emp_name")
    obj.date = request.POST.get("date")
    obj.hours = request.POST.get("hours")
    obj.amount = request.POST.get("amount")
    obj.save()
    return redirect("/Overtime_details/")
def overtime_edit_page(request,id):

    obj = get_object_or_404(Overtime,pk = id)
    obj_emp = Employee.objects.all()
    context = {
        'emp' : obj_emp,
        'ovr' : obj
    }
    return render(request,'overtime_edit_page.html',context)

def overtime_delete(request,id):
    try:
        obj = get_object_or_404(Overtime,pk = id)
        obj.delete()
        return redirect('/Overtime_details/')
...
def overtime_edit_save(request):
    id = request.POST.get("id")
    obj = get_object_or_404(Overtime,pk=id)
    obj.employee_id = request.POST.get("emp_name")
    obj.date = request.POST.get("date")
...
def purchase_edit(request, id):
    obj = get_object_or_404(purchased_products, pk=id)
    pur_prods = purchased_products.objects.filter(vendor_id=obj.vendor_id,
                                                  purchased_prod_reciept_date=obj.purchased_prod.reciept_date,
                                                  purchased_prod_bill_no=obj.purchased_prod.bill_no)
...
def invoice_page(request, id):
    ...
    x = purchased_products.objects.filter(vendor_id=obj.vendor_id,
                                          purchased_prod_reciept_date=obj.purchased_prod.reciept_date,
                                          purchased_prod_bill_no=obj.purchased_prod.bill_no)
...
def ajax_view(request):
    if request.method == 'POST':
        ...
        latest_product = purchased_products.objects.filter(vendor_id=selected_option).order_by(
            '-purchased_prod__reciept_date').first()
...
def invoice(request, id):
    ...
    price = obj
    return render(request, 'sale/invoice.html',
                  {'sale': obj, 'prod': obj2, 'total_tamount': total_tamount, 'total_dis': total_dis, 'bal': bal})
